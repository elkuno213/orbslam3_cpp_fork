...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            auto time_StartProcessKF = std::chrono::steady_clock::now();
#endif
            // BoW conversion and insertion in Map
            ProcessNewKeyFrame();
#ifdef REGISTER_TIMES
            auto time_EndProcessKF = std::chrono::steady_clock::now();
            ...
            MapPointCulling();
#ifdef REGISTER_TIMES
            auto time_EndMPCulling = std::chrono::steady_clock::now();
...
#ifdef REGISTER_TIMES
            auto time_EndMPCreation = std::chrono::steady_clock::now();
                ...
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        ...
                        if(!mpCurrentKeyFrame->GetMap()->GetIniertialBA2())
                        {
                            ...
                            {
                                LOG(WARNING) << "Not enough motion for initializing. Reseting...";
...
#ifdef REGISTER_TIMES
                auto time_EndLBA = std::chrono::steady_clock::now();
...
#ifdef REGISTER_TIMES
                auto time_EndKFCulling = std::chrono::steady_clock::now();
...
#ifdef REGISTER_TIMES
            auto time_EndLocalMap = std::chrono::steady_clock::now();
...
void LocalMapping::SearchInNeighbors()
{
    ...
    for(auto vit=vpNeighKFs.cbegin(), vend=vpNeighKFs.cend(); vit!=vend; vit++)
    {
        KeyFrame* pKFi = *vit;
    ...
    for(int i=0, imax=vpTargetKFs.size(); i<imax; i++)
    {
        ...
        for(auto vit2=vpSecondNeighKFs.cbegin(), vend2=vpSecondNeighKFs.cend(); vit2!=vend2; vit2++)
        {
            KeyFrame* pKFi2 = *vit2;
...
void LocalMapping::RequestReset()
{
    {
        std::unique_lock<std::mutex> lock(mMutexReset);
        LOG(INFO) << "LM: Map reset recieved";
        mbResetRequested = true;
    }
    LOG(INFO) << "LM: Map reset, waiting...";
...
void LocalMapping::RequestResetActiveMap(Map* pMap)
{
    {
        std::unique_lock<std::mutex> lock(mMutexReset);
        LOG(INFO) << "LM: Active map reset recieved";
        mbResetRequestedActiveMap = true;
        mpMapToReset = pMap;
    }
    LOG(INFO) << "LM: Active map reset, waiting...";
    ...
    LOG(INFO) << "LM: Active map reset, Done!!!";
}
...
void LocalMapping::ResetIfRequested()
{
    bool executed_reset = false;
    {
        ...
        if(mbResetRequested)
        {
            ...
            LOG(INFO) << "LM: Reseting Atlas in Local Mapping...";
            mlNewKeyFrames.clear();
            ...
            LOG(INFO) << "LM: End reseting Local Mapping...";
        }

        if(mbResetRequestedActiveMap) {
            executed_reset = true;
            LOG(INFO) << "LM: Reseting current map in Local Mapping...";
            mlNewKeyFrames.clear();
            mlpRecentAddedMapPoints.clear();
    ...
    if(executed_reset)
        LOG(INFO) << "LM: Reset free the mutex";
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    while(!lpKFtoCheck.empty())
    {
        ...
        if(pKF->bImu)
        {
            ...
            LOG(WARNING) << "KF " << pKF->mnId << " not set to inertial!!";
        }
