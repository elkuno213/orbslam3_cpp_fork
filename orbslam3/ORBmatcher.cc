...
namespace ORB_SLAM3
{
    ...
    int ORBmatcher::SearchForTriangulation(KeyFrame *pKF1, KeyFrame *pKF2,
                                           std::vector<std::pair<std::size_t, std::size_t> > &vMatchedPairs, const bool bOnlyStereo, const bool bCoarse)
    {
        const DBoW2::FeatureVector &vFeatVec1 = pKF1->mFeatVec;
        const DBoW2::FeatureVector &vFeatVec2 = pKF2->mFeatVec;
            ...
            if(f1it->first == f2it->first)
            {
                for(std::size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    for(std::size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                    {
                        ...
                        if(bCoarse || pCamera1->checkEpipolarConstrain(*pCamera2,kp1,kp2,R12,t12,pKF1->mvLevelSigma2[kp1.octave],pKF2->mvLevelSigma2[kp2.octave])) // MODIFICATION_2
                        {
                            bestIdx2 = idx2;
                            bestDist = dist;
                        }
        ...
        for(std::size_t i=0, iend=vpMPs.size(); i<iend; i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad() && !sAlreadyFound.count(pMP))
                {
                    ...
                    const std::vector<std::size_t> vIndices2 = CurrentFrame.GetFeaturesInArea(uv(0), uv(1), radius, nPredictedLevel-1, nPredictedLevel+1);
