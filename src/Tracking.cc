...
void Tracking::newParameterLoader(Settings* settings) {
  ...
  if (IsStereoBased(mSensor) || IsRGBDBased(mSensor)) {
    mbf      = settings->bf();
...
bool Tracking::ParseCamParamFile(cv::FileStorage& fSettings) {
  ...
  if (sCameraName == "PinHole") {
    ...
    if (IsStereoBased(mSensor) || mSensor == Sensor::InertialRGBD) {
      // Right camera
      // Camera calibration parameters
...
Sophus::SE3f Tracking::GrabImageStereo(
  ...
  } else if (mSensor == Sensor::InertialStereo && !mpCamera2) {
    mCurrentFrame = Frame(
      mImGray,
      imGrayRight,
      timestamp,
...
Sophus::SE3f Tracking::GrabImageMonocular(
  ...
  if (mSensor == Sensor::Monocular) {
    if (mState == TrackingState::NotInitialized || mState == TrackingState::NoImagesYet || (lastID - initID) < mMaxFrames) {
      mCurrentFrame = Frame(
        mImGray,
        timestamp,
        mpIniORBextractor,
    ...
    if (mState == TrackingState::NotInitialized || mState == TrackingState::NoImagesYet) {
      mCurrentFrame = Frame(
        mImGray,
        timestamp,
        mpIniORBextractor,
  ...
  if (mState == TrackingState::NoImagesYet) {
    t0 = timestamp;
  }
...
void Tracking::Track() {
    ...
    if (mLastFrame.mTimeStamp > mCurrentFrame.mTimeStamp) {
      ...
      if (mpAtlas->isInertial()) {
        if (mpAtlas->isImuInitialized()) {
          _logger->warn(
            "Timestamp jump detected after IMU initialization. Setting tracking state "
            "to Lost and resetting IMU integration...",
  ...
  if (IsInertialBased(mSensor) && mpLastKeyFrame) {
    mCurrentFrame.SetNewBias(mpLastKeyFrame->GetImuBias());
  }

  if (mState == TrackingState::NoImagesYet) {
    mState = TrackingState::NotInitialized;
  }

  mLastProcessedState = mState;

  if (IsInertialBased(mSensor) && !mbCreatedMap) {
#ifdef REGISTER_TIMES
  ...
  if (mState == TrackingState::NotInitialized) {
    ...
    if (mState != TrackingState::Tracking) { // If rightly initialized, mState=OK
    ...
    if (!mbOnlyTracking) {
      ...
      if (mState == TrackingState::Tracking) {
        ...
        if (!bOK) {
          if (mCurrentFrame.mnId <= (mnLastRelocFrameId + mnFramesToResetIMU) && IsInertialBased(mSensor)) {
        ...
        if (mState == TrackingState::RecentlyLost) {
    ...
    if (bOK) {
      ...
      if (IsInertialBased(mSensor)) {
        ...
        mState = TrackingState::RecentlyLost; // visual to lost
      }
    ...
    if ((mCurrentFrame.mnId<(mnLastRelocFrameId+mnFramesToResetIMU)) && (mCurrentFrame.mnId > mnFramesToResetIMU) && IsInertialBased(mSensor) && pCurrentMap->isImuInitialized()) {
    ...
    if (bOK || mState == TrackingState::RecentlyLost) {
      // Update motion model
      ...
      std::ranges::for_each(mlpTemporalPoints, [](MapPoint* const mp) {
        delete mp;
      });
      ...
      // if(bNeedKF && bOK)
      if (bNeedKF && (bOK || (mInsertKFsLost && mState == TrackingState::RecentlyLost && IsInertialBased(mSensor)))) {
        CreateNewKeyFrame();
      }
  ...
  if (mState == TrackingState::Tracking || mState == TrackingState::RecentlyLost) {
...
void Tracking::StereoInitialization() {
  if (mCurrentFrame.N > 500) {
    if (mSensor == Sensor::InertialStereo || mSensor == Sensor::InertialRGBD) {
...
void Tracking::MonocularInitialization() {
  if (!mbReadyToInitializate) {
    ...
    if (mCurrentFrame.mvKeys.size() > 100) {
      ...
      mLastFrame    = Frame(mCurrentFrame);
      // TODO(VuHoi): use std::ranges::transform?
      mvbPrevMatched.resize(mCurrentFrame.mvKeysUn.size());
      std::ranges::transform(
        mCurrentFrame.mvKeysUn,
        mvbPrevMatched.begin(),
    ...
    if (((int)mCurrentFrame.mvKeys.size() <= 100) || ((mSensor == Sensor::InertialMonocular) && (mLastFrame.mTimeStamp-mInitialFrame.mTimeStamp > 1.0))) {
      mbReadyToInitializate = false;
...
void Tracking::CreateInitialMapMonocular() {
  ...
  for (MapPoint* const mp : vpAllMapPoints) {
    if (mp) {
      mp->SetWorldPos(mp->GetWorldPos() * invMedianDepth);
...
void Tracking::CreateMapInAtlas() {
  ...
  if (IsInertialBased(mSensor) && mpImuPreintegratedFromLastKF) {
    delete mpImuPreintegratedFromLastKF;
    mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(), *mpImuCalib);
  }
...
void Tracking::UpdateLastFrame() {
  ...
  if (mnLastKeyFrameId == mLastFrame.mnId || IsMonocularBased(mSensor) || !mbOnlyTracking) {
    return;
  }
...
bool Tracking::TrackWithMotionModel() {
  ...
  int nmatches
    = matcher.SearchByProjection(mCurrentFrame, mLastFrame, th, IsMonocularBased(mSensor));
...
bool Tracking::TrackLocalMap() {
  ...
  if (!mpAtlas->isImuInitialized()) {
    ...
    if (mCurrentFrame.mnId <= mnLastRelocFrameId + mnFramesToResetIMU) {
      ...
      // if(!mbMapUpdated && mState == State::OK) //  && (mnMatchesInliers>30))
  ...
  if ((mnMatchesInliers > 10) && (mState == TrackingState::RecentlyLost)) {
    return true;
  }
  ...
  } else if (mSensor == Sensor::InertialStereo || mSensor == Sensor::InertialRGBD) {
...
bool Tracking::NeedNewKeyFrame() {
  if (IsInertialBased(mSensor) && !mpAtlas->GetCurrentMap()->isImuInitialized()) {
    if (mSensor == Sensor::InertialMonocular && (mCurrentFrame.mTimeStamp - mpLastKeyFrame->mTimeStamp) >= 0.25) {
      return true;
    } else if ((mSensor == Sensor::InertialStereo || mSensor == Sensor::InertialRGBD) && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp) >= 0.25) {
      return true;
    } else {
      return false;
    }
  ...
  if(nClosedPoints < thStereoClosedPoints && (mSensor==Sensor::Stereo ||
  mSensor==Sensor::InertialStereo))
  {
  ...
  const bool c1c = mSensor != Sensor::Monocular && mSensor != Sensor::InertialMonocular
                && mSensor != Sensor::InertialStereo && mSensor != Sensor::InertialRGBD
  ...
  bool c4 = false;
  if ((((mnMatchesInliers < 75) && (mnMatchesInliers > 15)) || mState == TrackingState::RecentlyLost) && (mSensor == Sensor::InertialMonocular)) { // MODIFICATION_2, originally ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RecentlyLost) && ((mSensor == Sensor::InertialMonocular)))
    c4 = true;
  } else {
    c4 = false;
  }
...
void Tracking::CreateNewKeyFrame() {
  ...
  if (!IsMonocularBased(mSensor)) { // TODO check if including imu_stereo
    ...
    if (!vDepthIdx.empty()) {
      ...
      for (const auto& [depth, i] : vDepthIdx) {
        ...
        if (depth > mThDepth && nPoints > maxPoint) {
          break;
        }
      }
    }
...
void Tracking::SearchLocalPoints() {
  ...
  for (MapPoint* const mp : mvpLocalMapPoints) {
    if (mp->mnLastFrameSeen == mCurrentFrame.mnId) {
      continue;
    }
    if (mp->isBad()) {
      continue;
    ...
    if (mp->mbTrackInView) {
      mCurrentFrame.mmProjectPoints[mp->mnId] = cv::Point2f(mp->mTrackProjX, mp->mTrackProjY);
    }
  }
  ...
  if (nToMatch > 0) {
    ...
    } else if (!mpAtlas->isImuInitialized() && IsInertialBased(mSensor)) {
      th = 10;
    }
    ...
    if (mState == TrackingState::Lost || mState == TrackingState::RecentlyLost) { // Lost for less
                                                                                  // than 1 second
      th = 15;
    }
...
void Tracking::UpdateLocalPoints() {
  ...
  for (KeyFrame* const pKF : mvpLocalKeyFrames | std::views::reverse) {
    ...
    for (MapPoint* const mp : vpMPs) {
      ...
      if (mp->mnTrackReferenceForFrame == mCurrentFrame.mnId) {
        continue;
      }
      if (!mp->isBad()) {
        count_pts++;
        mvpLocalMapPoints.push_back(mp);
        mp->mnTrackReferenceForFrame = mCurrentFrame.mnId;
      }
    }
  }
}
...
void Tracking::UpdateLocalKeyFrames() {
  ...
  if (!mpAtlas->isImuInitialized() || (mCurrentFrame.mnId < mnLastRelocFrameId + 2)) {
    for (int i = 0; i < mCurrentFrame.N; i++) {
      ...
      if (pMP) {
        if (!pMP->isBad()) {
          // TODO(VuHoi): the results are always 1 because std::map does not allow duplicates in
          // observations.
          const auto observations = pMP->GetObservations();
          std::ranges::for_each(observations | std::views::keys, [&](KeyFrame* const kf) {
            keyframeCounter[kf]++;
          });
        } else {
          mCurrentFrame.mvpMapPoints[i] = nullptr;
  ...
  // shares most points
  for (const auto& [kf, count] : keyframeCounter) {
    if (kf->isBad()) {
      continue;
    }
    ...
    mvpLocalKeyFrames.push_back(kf);
    kf->mnTrackReferenceForFrame = mCurrentFrame.mnId;
  }

  // Include also some not-already-included keyframes that are neighbors to already-included
  // keyframes
  auto it_end = mvpLocalKeyFrames.cend();
  for (auto it = mvpLocalKeyFrames.cbegin(); it != it_end; it++) {
    // Limit the number of keyframes
    if (mvpLocalKeyFrames.size() > 80) { // 80
      break;
    }

    KeyFrame* local_kf = *it;

    for (KeyFrame* const neighbor_kf : local_kf->GetBestCovisibilityKeyFrames(10)) {
      if (!neighbor_kf->isBad()) {
        if (neighbor_kf->mnTrackReferenceForFrame != mCurrentFrame.mnId) {
          mvpLocalKeyFrames.push_back(neighbor_kf);
          neighbor_kf->mnTrackReferenceForFrame = mCurrentFrame.mnId;
          break;
        }
      }
    }

    for (KeyFrame* const child_kf : local_kf->GetChilds()) {
      if (!child_kf->isBad()) {
        if (child_kf->mnTrackReferenceForFrame != mCurrentFrame.mnId) {
          mvpLocalKeyFrames.push_back(child_kf);
          child_kf->mnTrackReferenceForFrame = mCurrentFrame.mnId;
          break;
        }
      }
    }

    KeyFrame* pParent = local_kf->GetParent();
    if (pParent) {
  ...
  if (IsInertialBased(mSensor) && mvpLocalKeyFrames.size() < 80) {
...
void Tracking::ResetActiveMap(bool bLocMap) {
  ...
  mState = TrackingState::NoImagesYet; // State::NotInitialized;
  ...
  for (Map* const map : mpAtlas->GetAllMaps()) {
    if (map->GetAllKeyFrames().size() > 0) {
      if (index > map->GetLowerKFID()) {
        index = map->GetLowerKFID();
      }
    }
  }
